...
def load_eva_data(logger, args):
    ...
    es_ill = test_ill[:int(len(test_ill)*.3)]
    def remove_es_triple(es_ill, triples):
        triples = [t for t in triples if t[0] not in es_ill[:, 1] and t[2] not in es_ill[:, 1]]
        return triples
    if args.es:
        triples = remove_es_triple(es_ill, triples)
    ...
    att_features, attr2id = load_attr([a1, a2], ENT_NUM, ent2id_dict, 1000)  # attr
